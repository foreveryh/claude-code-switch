...
Writes provider configuration to .claude/settings.local.json
...
def get_project_settings_path() -> Path:
    """Get the path to project-level settings."""
...
def write_project_settings(
    ...
    """Write project-level settings for a provider.
    ...
    if provider.regions:
        try:
            ...
            console.print(f"[yellow]💡 Usage: ccm project {provider_name} [global|china][/yellow]")
            return False
    ...
    console.print(f"[green]✅ Wrote project settings for {canonical_name}{region_str}[/green]")
    console.print(f"[blue]   File: {settings_path}[/blue]")
    console.print(f"[yellow]💡 This overrides user settings for this project only.[/yellow]")
    console.print(f"[yellow]💡 Use 'ccm project reset' to remove.[/yellow]")

    return True


def reset_project_settings() -> bool:
    """Remove project-level settings."""
    settings_path = get_project_settings_path()

    if not settings_path.exists():
        console.print(f"[yellow]⚠️  No project settings to reset at: {settings_path}[/yellow]")
        return True
    ...
    console.print(f"[green]✅ Removed project settings: {settings_path}[/green]")
    console.print(f"[yellow]💡 Claude Code will fall back to user settings.[/yellow]")

    return True


def show_project_settings() -> None:
    """Show current project settings."""
    settings_path = get_project_settings_path()

    if not settings_path.exists():
        console.print("[yellow]No project settings configured.[/yellow]")
        console.print("[blue]Use 'ccm project <provider> [region]' to set.[/blue]")
        return

    try:
        data = json.loads(settings_path.read_text())

        console.print(f"[bold]Project settings:[/bold] {settings_path}")
        console.print()

...
show_help() {
    ...
    echo ""
    echo -e "${YELLOW}EVO Identity Module:${NC}"
    echo "  evo project enable    - Inject EVO.IA persona into project settings"
    echo "  evo project disable   - Remove EVO from project settings"
    echo "  evo user enable       - Inject EVO.IA persona into global settings"
    echo "  evo user disable      - Remove EVO from global settings"
    echo "  evo status            - Show EVO activation status"
    echo "  evo show              - Print the EVO system prompt"
    echo ""
...
# ============================================
# EVO Identity Module (System Prompt Manager)
# ============================================

evo_prompt_path() {
    # Locate evo-identity.txt relative to this script
    local prompt_file="$SCRIPT_DIR/prompts/evo-identity.txt"
    if [[ ! -f "$prompt_file" ]]; then
        echo -e "${RED}❌ EVO prompt file not found: $prompt_file${NC}" >&2
        return 1
    fi
    echo "$prompt_file"
}
...
evo_project_enable() {
    ...
    local prompt_file
    prompt_file="$(evo_prompt_path)" || return 1
    local prompt_content
    prompt_content="$(cat "$prompt_file")"

    # Backup if not ccm-managed
    if [[ -f "$settings_path" ]]; then
        if ! grep -q '"ccmManaged"[[:space:]]*:[[:space:]]*true' "$settings_path" 2>/dev/null; then
            backup_project_settings "$settings_path"
        fi
...
PYTHON_EOF
        ...
        echo -e "${RED}❌ python3 is required to write the EVO system prompt.${NC}" >&2
        return 1
    fi

    echo -e "${GREEN}✅ EVO Identity enabled for this project.${NC}" >&2
    echo -e "${BLUE}   File: $settings_path${NC}" >&2
    echo -e "${YELLOW}💡 Use 'ccm evo project disable' to remove.${NC}" >&2
}
...
evo_project_disable() {
    ...
    if [[ ! -f "$settings_path" ]]; then
        echo -e "${YELLOW}ℹ️  No project settings file found.${NC}" >&2
        return 0
    fi
...
existing.pop('systemPrompt', None)
existing.pop('ccmEvoIdentity', None)
...
PYTHON_EOF
        ...
        echo -e "${RED}❌ python3 is required to update settings.${NC}" >&2
        return 1
    fi

    echo -e "${GREEN}✅ EVO Identity removed from project settings.${NC}" >&2
    echo -e "${BLUE}   File: $settings_path${NC}" >&2
}
...
PYTHON_EOF
    ...
    echo -e "${GREEN}✅ EVO Identity enabled globally (user settings).${NC}" >&2
    echo -e "${BLUE}   File: $settings_path${NC}" >&2
    echo -e "${YELLOW}💡 Use 'ccm evo user disable' to remove.${NC}" >&2
}
...
evo_user_disable() {
    ...
    if [[ ! -f "$settings_path" ]]; then
        echo -e "${YELLOW}ℹ️  No user settings file found.${NC}" >&2
        return 0
    fi
...
PYTHON_EOF
    ...
    echo -e "${GREEN}✅ EVO Identity removed from user settings.${NC}" >&2
    echo -e "${BLUE}   File: $settings_path${NC}" >&2
}
...
evo_show_status() {
    ...
    echo -e "${BLUE}🤖 EVO Identity Module Status${NC}"
    echo ""

    # Project-level
    if [[ -f "$project_settings" ]] && grep -q '"ccmEvoIdentity"[[:space:]]*:[[:space:]]*true' "$project_settings" 2>/dev/null; then
        echo -e "  Project: ${GREEN}✅ ENABLED${NC}  ($project_settings)"
    else
        echo -e "  Project: ${YELLOW}○  disabled${NC}"
    fi

    # User-level
    if [[ -f "$user_settings" ]] && grep -q '"ccmEvoIdentity"[[:space:]]*:[[:space:]]*true' "$user_settings" 2>/dev/null; then
        echo -e "  User:    ${GREEN}✅ ENABLED${NC}  ($user_settings)"
    else
        echo -e "  User:    ${YELLOW}○  disabled${NC}"
    fi

    echo ""
    local prompt_file
    if prompt_file="$(evo_prompt_path 2>/dev/null)"; then
        echo -e "  Prompt file: ${BLUE}$prompt_file${NC}"
    else
        echo -e "  Prompt file: ${RED}not found${NC}"
    fi
}

evo_show_usage() {
    echo -e "${BLUE}🤖 EVO Identity Module — System Prompt Manager${NC}" >&2
    echo "" >&2
    echo "Injects the EVO.IA persona + security rules into Claude Code settings" >&2
    echo "via the systemPrompt key (Modules 1 & 5 of the EVO.IA system prompt)." >&2
    echo "" >&2
    echo -e "${YELLOW}Usage:${NC}" >&2
    echo "  ccm evo project enable    - Enable for current project only" >&2
    echo "  ccm evo project disable   - Remove from current project" >&2
    echo "  ccm evo user enable       - Enable globally (all projects)" >&2
    echo "  ccm evo user disable      - Remove from global settings" >&2
    echo "  ccm evo status            - Show current EVO status" >&2
    echo "  ccm evo show              - Print the system prompt content" >&2
    echo "" >&2
    echo -e "${YELLOW}Examples:${NC}" >&2
    echo "  ccm evo project enable    # Activate EVO for this project" >&2
    echo "  ccm evo user enable       # Activate EVO for all projects" >&2
    echo "  ccm evo status            # Check if EVO is active" >&2
    echo "" >&2
    echo -e "${YELLOW}Note:${NC}" >&2
    echo "  Project settings take priority over user settings." >&2
    echo "  Prompt file: \$CCM_DIR/prompts/evo-identity.txt" >&2
}
    ...
    case "$cmd" in
        ...
        "evo")
            ...
            case "$evo_scope" in
                "project")
                    case "$evo_action" in
                        "enable")  evo_project_enable ;;
                        "disable") evo_project_disable ;;
                        *)
                            echo -e "${RED}❌ Unknown EVO project action: $evo_action${NC}" >&2
                            evo_show_usage
                            return 1
                            ;;
                    esac
                ...
                "user")
                    case "$evo_action" in
                        "enable")  evo_user_enable ;;
                        "disable") evo_user_disable ;;
                        *)
                            echo -e "${RED}❌ Unknown EVO user action: $evo_action${NC}" >&2
                            evo_show_usage
                            return 1
                            ;;
                    esac
                ...
                ""|"help"|"-h"|"--help")
                    ...
                    ;;
                *)
                    echo -e "${RED}❌ $(t 'unknown_option'): evo $evo_scope${NC}" >&2
                    evo_show_usage
                    return 1
                    ;;
            esac
